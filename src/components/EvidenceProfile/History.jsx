...
const History = () => {
    ...
    useEffect(() => {
      ...
      // 2. Filter by Location (Simple substring match for now)
    ...
    return (
        <div className='flex-1 overflow-auto bg-white'>
            ...
            <div className="md:hidden">
                ...
                {/* Cards */}
                 <div className="pr-4 pb-14 pl-4 grid grid-cols-1 gap-4">
                    {currentItems.length > 0 ? (
                         currentItems.map((item) => (
                         <div key={item.id || item.name + item.date} className="border rounded-lg p-4 shadow bg-white flex flex-col items-start space-y-2 relative">
                            <img src={item.image} alt={item.name} className="w-full h-32 object-contain mb-2 self-center" />
                            ...
                            <div className="flex items-center gap-2 text-gray-600 text-sm"> <FiCalendar /> {item.date} </div>
                            <div className="flex items-center gap-2 text-gray-600 text-sm"> <FiMapPin /> {item.location} </div>
                            <div className="pt-2 w-full flex justify-end"> <button className="px-3 py-1 bg-[#7a0000] text-white rounded hover:bg-[#5a0000] text-xs"> ดูรายละเอียด </button> </div>
                         </div>
                        ))
                                ...
                                <table className="w-full table-fixed border-collapse">
                                    <thead>
                                        <tr className="bg-gray-200 sticky top-0 z-10">
                                            <th className="p-3 text-left w-[15%] font-semibold">วัน/เดือน/ปี</th> <th className="p-3 text-left w-[15%] font-semibold">หมวดหมู่</th> <th className="p-3 text-left w-[10%] font-semibold">รูปภาพ</th> <th className="p-3 text-left w-[20%] font-semibold">ชื่อ</th> <th className="p-3 text-left w-[25%] font-semibold">สถานที่พบ</th> <th className="p-3 text-left w-[15%] font-semibold">การจัดการ</th>
                                        </tr>
                                    </thead>
                                    {currentItems.length > 0 ? (
                                        <tbody>
                                            {currentItems.map((item) => (
                                                <tr key={item.id || item.name + item.date} className="border-t hover:bg-red-50 transition-colors">
                                                    <td className="p-3 align-top">{item.date}</td> <td className="p-3 align-top">{item.category}</td> <td className="p-3 align-top"> <img src={item.image} alt={item.name} className="w-12 h-12 object-contain" /> </td> <td className="p-3 align-top">{item.name}</td> <td className="p-3 align-top">{item.location}</td> <td className="p-3 align-top"> <div className="flex gap-1"> <button title="ดูรายละเอียด" className="p-1 text-blue-600 hover:text-blue-800 hover:bg-blue-100 rounded"><FiEye size={18} /></button> <button title="แก้ไข" className="p-1 text-yellow-600 hover:text-yellow-800 hover:bg-yellow-100 rounded"><FiEdit size={18} /></button> <button title="ลบ" className="p-1 text-red-600 hover:text-red-800 hover:bg-red-100 rounded"><FiTrash size={18} /></button> </div> </td>
                                                </tr>
                                            ))}
                                        </tbody>
