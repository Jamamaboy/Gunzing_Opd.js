...
const GunCatalogAdmin = () => {
    ...
    useEffect(() => {
        ...
        fetch(`${apiConfig.baseUrl}${API_PATH}/exhibits`)
            .then(res => res.json())
            .then(data => {
                console.log('API response (exhibits):', data);
                // แปลงโครงสร้างข้อมูลจาก API ใหม่ให้เข้ากับโครงสร้างเดิม
                const mapped = (Array.isArray(data) ? data : [])
                    .filter(item => item.firearm) // กรองเฉพาะ item ที่มีข้อมูล firearm
                    .map(item => ({
                        id: item.id,
        ...
        <div className='h-full w-full bg-[#F8F9FA] flex flex-col overflow-hidden'>
            ...
            <div className="px-6 py-4 flex justify-between items-center flex-shrink-0">
                ...
                <div className='flex items-center space-x-2'>
                    ...
                    {/* ปุ่มเพิ่มอาวุธปืน */}
                    <button
                        onClick={() => navigate('/admin/guns/create-gun')}
                        className="px-4 py-2 bg-[#990000] text-white rounded-lg font-medium hover:bg-red-800 transition-colors"
                    >
                        + เพิ่มอาวุธปืน
                    </button>
                </div>
        ...
        <div className='h-full w-full bg-[#F8F9FA] flex flex-col overflow-hidden'>
            ...
            <div className="px-4 py-3 flex justify-between items-center flex-shrink-0">
                ...
                <div className='flex items-center space-x-2'>
                    ...
                    {/* ปุ่มเพิ่มอาวุธปืน (Mobile) */}
                    <button
                        onClick={() => navigate('/admin/guns/create-gun')}
                        className="px-3 py-1.5 bg-[#990000] text-white text-xs rounded-lg font-medium hover:bg-red-800 transition-colors"
                    >
                        +
                    </button>
                </div>
...
export default GunCatalogAdmin;